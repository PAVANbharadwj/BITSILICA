...
#define OPT3001_ADDR   (0x47)   // HAL uses 8-bit address
...
static void opt3001_init(void)
{
    OPT3001_Write(OPT3001_ADDR, OPT3001_CONFIG_REG, 0xC410);
}
...
static float opt3001_read_lux(void)
{
    uint16_t raw = OPT3001_ReadReg(OPT3001_ADDR, OPT3001_RESULT_REG);

    uint16_t exp  = (raw >> 12) & 0x0F;
    uint16_t mant = raw & 0x0FFF;

    return (0.01 * (1 << exp) * mant);
}
